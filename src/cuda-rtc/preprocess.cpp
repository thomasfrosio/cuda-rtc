...
void write_binary_as_c_array(
    ...
    ostream << "// This file is automatically generated\n\n";
    ...
    bool parse(int argc, char* argv[]) {
        ...
        while ((arg_c = *++argv)) {
            ...
            if (arg[0] == '-') {
                ...
                } else if (arg == "-p" || arg == "--namespace") {
                    arg_c = *++argv;
                    if (!arg_c) {
                        std::cerr << "Expected prefix after --namespace" << std::endl;
                        return false;
                    }
                    namespace_name = arg_c;
...
int main(int argc, char* argv[]) {
    ...
    for (const std::string& source_filename : options.source_filenames) {
        ...
        // Serialize the preprocessed source and save it in a C++ source.
        ...
        std::string output_filename = path_join(options.output_dir, source_filename + ".jit.cpp");
        if (!make_directories_for(output_filename))
            return EXIT_FAILURE;
        std::ofstream file(output_filename, std::ios::binary);
        write_binary_as_c_array(source_varname, ss, file, options.namespace_name);
        if (!file) {
    ...
    if (share_headers) {
        ...
        if (!make_directories_for(output_filename))
            return EXIT_FAILURE;
        std::ofstream file(output_filename, std::ios::binary);
        write_binary_as_c_array(shared_headers_varname, ss, file, options.namespace_name);
        if (!file) {
            std::cerr << "Error writing output to " << output_filename << std::endl;
            return EXIT_FAILURE;
        }
